@php
    ...
    for($i = 0; $i < $events->count();$i+=2) {
        $arr[] = $events->slice($i, 2);
    }
@endphp

@foreach($arr as $events)
<tr>
        <td width="640" align="center" style="font-family: 'Montserrat', sans-serif;
                                                font-style: normal;
                                                font-weight: normal;">
            @foreach($events as $key => $event)
                <table width="{{count($events) > 1 ? '260' : '278'}}" class="{{count($events) > 1 ? (($key % 2) == 0 ? 'event-card-left' : 'event-card-right') : 'event-card-center'}}" border="0" cellpadding="0" cellspacing="0" bgcolor="ffffff">
                    <tr>
                        <td class="card" width="{{count($events) > 1 ? '260' : '278'}}" style="padding:5px;">
