            ...
            <table class="text">
                ...
                @foreach($events as $event)
                    ...
                    @foreach($event->comments as $comment)
                        ...
                        <!--   <td class="cols" >{{ $comment->created_at }}</td>
                              <td><a style="color:#727272;font-weight: normal;font-size:14px;" href="{{ route('cabinet.comment.show', $comment) }}">{{ $comment->text }}</a></td>
                        -->

                    @endforeach
                    <tr class="non eventitem">
                        <td class="cols">{{  date('Y-m-d', strtotime($date_reviews)) }}</td>
                        <td class="cols"><a
                                href="{{ route('frontend.event.show', $event) }}">{{ $event->name }}</a></td>
                        <td class="cols"><span title="{{$text_reviewslength}}"><a href="{{route('cabinet.comment.show', $comment)}}">{{$text_reviews}}</a></span></td>
                    </tr>
...
$text_reviewslength=$comment->text;
    $text_company = mb_strimwidth($comment->text, 0, 30, "...");
                    ...
                    <tr class="non companytitem">
                        ...
                        <td class="cols"><span title="{{$text_reviewslength}}"><a href="{{route('cabinet.comment.show', $comment)}}">{{$text_company}}</a></span></td>
                    </tr>
                @endforeach
        ...
        <div class=" d-flex justify-content-center">
        <div style="display:none;" class="col-lg-6 col-sm-8">
            {{ $companies->appends(['text' => Request::input('text'), 'date' => Request::input('date'), 'categories' => Request::input('categories'), 'location' => Request::input('location')])->links('frontend.includes.pagination', ['paginator' => $companies]) }}
        </div>
        <div class="col-lg-3 col-sm-4 show-more">
