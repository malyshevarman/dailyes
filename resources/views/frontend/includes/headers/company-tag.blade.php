...
@section('header-style', "background: linear-gradient(rgba(0, 0, 0, 0.7), rgba(0, 0, 0, 0.7)), url('$tag->background_url'); background-size: cover; background-position: center center;")
    ...
    <div class="top-block">
        <div class="top-block__breadcrumb">
            <ul>
                ...
                @if(empty($tag) && !request('selection'))
                    <li>
                        Компании
                    </li>
                ...
                @else
                    ...
                    <li>Компании категории {{ $tag->name }}</li>
                @endif
            </ul>
        </div>
        <h1>{!!str_replace(array("city", "tag"), array($city->name, $tag->name), $tag->companySeo->h1)!!}</h1>
        ...
        <div class="top-block__view-map">
            <a class="d-flex" @click="$bus.$emit('showCompaniesMap')"><svg style="margin-right: 8px;" width="20" height="25" viewBox="0 0 17 22" fill="none" xmlns="http://www.w3.org/2000/svg">
