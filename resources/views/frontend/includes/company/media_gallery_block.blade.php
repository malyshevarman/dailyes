                                    ...
                                    <div class="item">
                                        <div class="block-gallery">
                                            ...
                                            <a data-fancybox="gallery-mob"
                                               href="{{ $gallery_item->attachable_type == 'App\Video' ? $gallery_item->attachable->link : (isset($gallery_item->attachable) ? $gallery_item->attachable->url : '')}}">
                    ...
                    <div class="row block-gallery__global">
                        @foreach($company->gallery_items as $gallery_item)
                            @if($gallery_item->attachable_type == 'App\Video' || ($loop->iteration == 6 && $loop->remaining > 0))
                                <div class="col-md-4">
                                    <div class="block-gallery">
                                        <img class="block-gallery__photo" src="{{ isset($gallery_item->attachable->url) ? $gallery_item->attachable->url : '' }}"/>
                                        <a data-fancybox="gallery"
                                           href="{{ $gallery_item->attachable_type == 'App\Video' ? $gallery_item->attachable->link : (isset($gallery_item->attachable->url) ? $gallery_item->attachable->url : '')}}">
                                            <div class="block-gallery__shadow">
                                                <div class="h-align">
                                                    <div class="v-align">
                                                        @if($loop->iteration == 6 && $loop->remaining > 0)
                            ...
                            @else
                                <div class="col-md-4">
                                    <div class="block-gallery">
                                        <a data-fancybox="gallery" href="{{isset($gallery_item->attachable->url) ? $gallery_item->attachable->url : ''}}">
                                            <img class="block-gallery__photo"
                                                 src="{{isset($gallery_item->attachable->url) ? $gallery_item->attachable->url : ''}}"/>
                                        </a>
                                    </div>
                                </div>
                            @endif
                        ...
                        <div hidden>
                            @foreach($company->gallery_items as $gallery_item)
                                ...
                                @else
                                    ...
                                    @elseif($gallery_item->attachable_type == 'App\Download')
                                        <a data-fancybox="gallery"
                                           href="{{isset($gallery_item->attachable->url) ? $gallery_item->attachable->url : ''}}"></a>
                                    @endif
                                @endif
                            @endforeach
                        </div>
