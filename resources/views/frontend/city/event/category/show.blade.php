...
@section('content')
        ...
        <div class="container minus-15">
            <div class="row">
                <div class="col-md-12">
                        ...
                        <ul>
                            @if (isset($newTab[0]))
                                <li class="@if(request()->label_id == 1) active @endif">
                                    <a href="{{ route('frontend.city.event.category.show', ['label_id' => 1]) }}">Новые</a>
                                </li>
                            @endif
                            @if (isset($endTab[0]))
                    ...
                    @if(isset($requestInput) && count($requestInput) > 0)
                    <ul class="request-input" style="margin-top: 40px;">
                        @foreach ($requestInput as $key => $input)
                            @if ($key == 'Сортировано по:')
                                @if (Request::input('sort-raiting'))
                                    <li>
                                        <span class="filter_tag">
                                            <a href="{{route('frontend.event.index', Request::except('sort-raiting'))}}">
                                                {{ $input }} 
                                                @if(Request::input('sort-raiting') == 'asc') 
                                                    <i>&#8593;</i> 
                                                @else 
                                                    <i>&#8595;</i> 
                                                @endif
                                                <i class="clear_filter">&times;</i>
                                            </a>
                                        </span>
                                    </li>
                                @elseif (Request::input('sort-views'))
                                    <li>
                                        <span class="filter_tag">
                                            <a href="{{route('frontend.event.index', Request::except('sort-views'))}}">
                                                {{ $input }} 
                                                @if(Request::input('sort-views') == 'asc') 
                                                    <i>&#8593;</i> 
                                                @else 
                                                    <i>&#8595;</i> 
                                                @endif
                            ...
                            @else
                                @if ($key == 'По дате:')
                                    <li>
                                        <span class="filter_tag">
                                            <a href="{{route('frontend.event.index', Request::except('date'))}}">
                                                {{ $input }}
                                                <i class="clear_filter">&times;</i>
                                            </a>
                                        </span>
                                    </li>
                                @elseif($key == 'Категории:')
                                        @foreach($input as $cat)
                                        <li>
                                            <span class="filter_tag">
                                                <a href="{{route('frontend.event.index', Request::except('tags.' . array_search($cat['id'], Request::input('tags'))))}}">
                                                    {{ $cat['name'] }}
                                                    <i class="clear_filter">&times;</i>
                                                </a>
                                            </span>
                                ...
                                @elseif($key == 'Адрес:')
                                    <li>
                                        <span class="filter_tag">
                                            <a href="{{route('frontend.event.index', Request::except('location'))}}">
                                                {{ $input }}
                                                <i class="clear_filter">&times;</i>
                                            </a>
                                        </span>
                                ...
                                <div class="col-xl-3 col-lg-6 col-md-6 col-sm-6 event-item bootfix-col">
                                    ...
                                    @if($event->tags->count() > 0)
                                        <a href="{{ route('frontend.city.event.tag.show', $event->tags[0]) }}"
                                            ...
                                            <span class="events-block__group">{{!empty($tag) ? 
                                                $tag->name : 
                                                (
                                                    !empty(Request::input('tags')) ? 
                                                    $event->tags->first(function($item) {
                                                        return in_array($item->id, Request::input('tags'));
                                                    })->name : 
                                                    $event->tags[0]->name
                                                )}}</span>
                                        </a>
                            ...
                            <div class="col-md-12 bootfix-col">
                                <div class="row d-flex justify-content-center">
                                    <div style="display:none;" class="col-lg-6 col-sm-8">
                                        {{ $events->appends(Request::all())->links('frontend.includes.pagination', ['paginator' => $events]) }}
                                    </div>
    ...
    <div class="container seoblock">
        @if (!is_null(request()->get('label_id')) && request()->get('label_id') == 1)
            @include('frontend.includes.seo.new_events_page')
        @elseif (!is_null(request()->get('label_id')) && request()->get('label_id') == 2)
            @include('frontend.includes.seo.ends_events_page')
        @elseif (!is_null(request()->get('favorite')))
            @include('frontend.includes.seo.favorite_events_page')
        @elseif (!is_null(request()->get('sort-raiting')))
            @include('frontend.includes.seo.sort_rating_events_page')
        @elseif (!is_null(request()->get('sort-views')))
            @include('frontend.includes.seo.sort_views_events_page')
        @else
            @include('frontend.includes.seo.events_list_page')
        @endif
    </div>
    <div class="modal fade map-modal" id="map-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <event-modal-map-index
                ...
                :city="{{json_encode($city)}}"></event-modal-map-index>
        </div>
    </div>
@endsection
@push('after-scripts')
