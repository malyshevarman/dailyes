...
@section('title', $category->slide->name)
@section('description', $category->slide->summary)
@section('image', $category->slide->image_url ?? '')
@section('url', route('frontend.rating.company.show', $category->slide))
...
@section('content')
    ...
    <div class="container seoblock">
            @if (is_null($category->slide->text))
            @include('frontend.includes.seo.companies_rating_list_page')
            @else
                {!!str_replace(array("city", "category"), array($city->name, $category->name), $category->slide->text)!!}
            @endif
    </div>
