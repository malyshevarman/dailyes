	...
	<div class="map-content">
        <div class="map-content__header">
            ...
            <div class="map-content__header-middle-mobile">
            	...
            	<div class="search">
                    <form>
                        <input placeholder="Найти акцию" :value="requestText" name="text" type="text"/>
                        <button>
            ...
            <event-modal-map-header-categories :newTab="newTab" :endTab="endTab"></event-modal-map-header-categories>
        </div>
        <div class="modal-body">
            <div id="map-modal-close" class="close-modal" @click="closeModal"><i class="fas fa-times"></i></div>
            <event-modal-map-left-side-list-items></event-modal-map-left-side-list-items>
            <div id="map" style="width: 100%; height: 100vh;">
                <event-modal-map :city="city" :balloon="true" :drag="true" :modal="true"></event-modal-map>
            </div>
        </div>
    </div>
</template>
...
<script>
	...
	export default {
		...
		mounted() {
			...
			this.$bus.$on('getSelectedTagFromEventsModalHeaderComponent', (tag) => {
                    this.selectedTag = tag
                    // if (this.currentCategory !== null && Object.keys(this.currentCategory).length == 0) {
                    	// this.currentCategory = category
                    // }
                });
			this.$bus.$on('showEventsMap', () => {
					this.show()
                });
		},
		methods: {
			...
			closeModal() {
				if (this.selectedTag == null && this.currentTag == null || this.selectedTag !== null && JSON.stringify(this.selectedTag) === JSON.stringify({}) || this.currentTag !== null && this.selectedTag !== null && this.selectedTag.id == this.currentTag.id) {
					...
					if (this.selectedTag == null || JSON.stringify(this.selectedTag) === JSON.stringify({})) {
						...
						location.href = `/stocks/tag/${this.selectedTag.slug}`
					}
				}
			},
			fetch() {
