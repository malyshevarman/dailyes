...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            //         }
            //     })
                this.$bus.$on('fetchMapEventsAddresses', (addresses) => {
                    this.fetchMapEventsAddresses(addresses)
                });
            // }
        },
        methods: {
            fetchMapEventsAddresses(addresses) {
            ...
            setMarkers() {
                ...
                this.clusterer.events.add('click', (e) => {
                    ...
                    if (target.geometry && typeof target.getGeoObjects) {
                        ...
                        // }
                        // console.log(target.properties.get('events'))
                        if (target.properties.get('events')) {
                            this.$bus.$emit('fetchPlacemarkEvents', target.properties.get('events'), target.properties.get('address'))
                        }
                        // console.log("target.properties.get('events')", target.properties.get('events'))
            ...
            openBalloon(id) {
                ...
                this.placemarks.forEach((placemark) => {
                    ...
                    if (placemark.properties.get('id') === Number(id)) {
                        this.$bus.$emit('fetchPlacemarkEvents', placemark.properties.get('events'), placemark.properties.get('address'))
                        // console.log(id)
                        ymaps.geoQuery(placemark).setOptions('preset', 'map#open')
                        ...
                        this.map.panTo(placemark.geometry.getCoordinates())
                            .then(function () {
                                // Откроем балун на третьей метке в массиве.
                                // this.map.setCenter(placemark.geometry.getCoordinates(), 15, {checkZoomRange: true})
                                //     .then(() => {
