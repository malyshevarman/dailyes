    ...
    <a-layout-content :style="{ margin: '24px 16px', padding: '24px', background: '#fff', minHeight: '100%' }">
      <div :style="{ marginBottom: '24px', display: 'flex', justifyContent: 'space-between' }">
        ...
        <div>
          <a
            ...
            href="/admin/company/category/create"
      ...
      >
        <a
          ...
          :href="`/admin/company/category/${record.slug}/edit`"
        >
          <a-icon type="edit" />
        </a>
      </a-table>
...
<script>
    ...
    export default {
        ...
        mounted() {
            // if (localStorage.getItem("filtersTableCompanyCategories")) {
            //     this.filters = JSON.parse(localStorage.getItem("filtersTableCompanyCategories"))
            // } else {
            //     localStorage.setItem("filtersTableCompanyCategories", this.filters);
            // }
            this.fetchData()
        },
        methods: {
            fetchData() {
                ...
                // localStorage.setItem("filtersTableCompanyCategories", JSON.stringify(this.filters));

                axios.post(`/api/backend/company/category/filter?page=${this.filters.pagination.current_page}`, this.filters)
                    .then(response => {
