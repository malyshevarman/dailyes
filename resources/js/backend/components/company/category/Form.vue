        ...
        methods: {
            fetch() {
                if (!this.loading) {
                    ...
                    axios.get(`/api/backend/company/category/${res[4]}`)
                        .then(response => {
            ...
            update() {
                if (!this.loading) {
                    ...
                    axios.put(`/api/backend/company/category/${res[4]}`, this.category)
                        .then(response => {
                            this.errors = {}
                            this.category = response.data
            ...
            create() {
                if (!this.loading) {
                    this.loading = true
                    const message = this.$message.loading('Сохранение', 0)
                    axios.post(`/api/backend/company/category`, this.category)
                        .then(response => {
                            this.errors = {}
                            this.category = response.data
                            this.$message.success('Успешно сохранено!')
                            location.href = `/admin/company/category/${response.data.slug}/edit`
                        })
                        .catch(error => {
                            this.$message.error('Ошибка')
                            this.errors = error.response.data.errors
            ...
            destroy() {
                if (!this.loading) {
                    ...
                    swal({
                        ...
                        .then((willDelete) => {
                            if (willDelete) {
                                axios.delete(`/api/backend/company/category/${this.category.slug}`)
                                    .then(response => {
                                        location.replace(`/admin/company/category`)
                                    })
                                    .catch(error => {
                                        this.errors = error.response.data.errors
                                    })
