<template>
  ...
  <a-col :xs="{ span: 24, offset: 0 }" :lg="{ span: 8, offset: 2 }">
    <template v-if="address.lat && address.long">
      ...
      <a-button style="padding-left:0;" type="link" @click="isvisible=!isvisible" v-if="!isvisible">+ добавить телефон</a-button>
      <a-form-item
              label=""
      ...
      >
        ...
        <a-button-group style="margin-bottom: 10px;min-width: 350px;display: flex;">
            ...
            <a-button class="day-btn" :class="[activedn == 'mon' ? 'active': '', address.mon !='00:00-00:00' ? 'notnull':'']" @click="activedn = 'mon'">ПН</a-button>
            <a-button class="day-btn" :class="[activedn == 'tue' ? 'active': '', address.tues !='00:00-00:00' ? 'notnull':'']" @click="activedn = 'tue'">ВТ</a-button>
            <a-button class="day-btn" :class="[activedn == 'wed' ? 'active': '', address.wed !='00:00-00:00' ? 'notnull':'']" @click="activedn = 'wed'">СР</a-button>
            <a-button class="day-btn" :class="[activedn == 'thu' ? 'active': '', address.thurs !='00:00-00:00' ? 'notnull':'']" @click="activedn = 'thu'">ЧТ</a-button>
            <a-button class="day-btn" :class="[activedn == 'fri' ? 'active': '', address.fri !='00:00-00:00' ? 'notnull':'']" @click="activedn = 'fri'">ПТ</a-button>
            <a-button class="day-btn" :class="[activedn == 'sat' ? 'active': '', address.sat !='00:00-00:00' ? 'notnull':'']" @click="activedn = 'sat'">СБ</a-button>
            <a-button class="day-btn" :class="[activedn == 'sun' ? 'active': '', address.sun !='00:00-00:00' ? 'notnull':'']" @click="activedn = 'sun'">ВС</a-button>

        </a-button-group>

          <template v-if="activedn == 'mon'">
            <div class="components-input" style="    display: flex;    align-items: center;">
              <a-input v-model="address.mon" v-mask="'##:##-##:##'" />
            </div>
          </template>
            <template v-if="activedn == 'tue'">
                <div class="components-input" style="    display: flex;    align-items: center;">
                    <a-input v-model="address.tues" v-mask="'##:##-##:##'" />
                </div>
            </template>
            <template v-if="activedn == 'wed'">
                <div class="components-input" style="    display: flex;    align-items: center;">
                    <a-input v-model="address.wed" v-mask="'##:##-##:##'" />
                </div>
            </template>
            <template v-if="activedn == 'thu'">
                <div class="components-input" style="    display: flex;    align-items: center;">
                    <a-input v-model="address.thurs" v-mask="'##:##-##:##'" />
                </div>
            </template>
            <template v-if="activedn == 'fri'">
                <div class="components-input" style="    display: flex;    align-items: center;">
                    <a-input v-model="address.fri" v-mask="'##:##-##:##'" />
                </div>
            </template>
            <template v-if="activedn == 'sat'">
                <div class="components-input" style="    display: flex;    align-items: center;">
                    <a-input v-model="address.sat" v-mask="'##:##-##:##'" />
                </div>
            </template>
            <template v-if="activedn == 'sun'">
                <div class="components-input" style="    display: flex;    align-items: center;">
                    <a-input v-model="address.sun" v-mask="'##:##-##:##'" />
                </div>
            </template>
