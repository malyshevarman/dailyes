  ...
  <div class="gallery">
    ...
    >
      <template v-for="(item, index) in galleryItems">
        <a-col :span="2" :class="changed.find(c => c == item.id) ? 'changed' : ''">
          <a-card
            hoverable
          >
            <img
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            changed() {
              let array = this.changes.filter(log => {return log.changed_data.hasOwnProperty('gallery_items')})
              return array.map(f => {
                return f.changed_data.gallery_items
              })
            },
        },
        methods: {
            // checkChanges() {
            //   let found = this.changes.filter(log => {return log.changed_data.hasOwnProperty('gallery_items')})
            //   // found.changed_data.gallery_items.find(gi => gi == item.id)
            //   console.log(found)
            // },
            handleOkDialog() {
                this.upload()
            ...
            upload() {
                ...
                reqwest({
                    url: '/api/backend/gallery-item/store-multiple',
                    method: 'post',
                    processData: false,
                    data: formData,
                    success: (response) => {
            ...
            deleteItem(i, item) {
                // this.counter--
                axios.get(`/api/backend/gallery-item/remove-multiple/${item.id}`)
                      .then(response => {
