        ...
        <a-layout-content :style="{ margin: '24px 16px', padding: '24px', background: '#fff', minHeight: '100%' }">
            ...
            >
                ...
                <div
                    ...
                    slot-scope="text, record"
                >
                    <a target="_blank" :href="`/company/${record.commented.slug}`">{{record.commented.name}}</a>
                </div>
...
<script>
    ...
    const columns2 = [
        ...
        {
            title: 'Компания', 
            dataIndex: 'commented.name', 
            key: 'commented.id',
            sorter: (a, b) => {
                if (a.commented.name > b.commented.name) {
                  return 1;
                }
                if (a.commented.name < b.commented.name) {
                  return -1;
    ...
    const EditableCell = {
        ...
        <div class="editable-cell">
            <div v-if="editable" class="editable-cell-input-wrapper">
                <a-textarea :autosize="true" :value="value" @change="handleChange" @pressEnter="check" /><a-icon
                    type="check"
    ...
    export default {
        ...
        mounted() {
            // if (localStorage.getItem("newfiltersTableComments")) {
            //     this.filters = JSON.parse(localStorage.getItem("newfiltersTableComments"))
            // } else {
            //     localStorage.setItem("newfiltersTableComments", this.filters);
            // }
            this.fetchData()
        },
        methods: {
            ...
            updateCommentText(key, dataIndex, value) {
                console.log(key)
                axios.post(`/api/backend/comment/${key}/update-text`, {text: value})
                    .then(response => {
            ...
            updateCommentRejectionMessage(key, dataIndex, value) {
                console.log(key)
                axios.post(`/api/backend/comment/${key}/update-message`, {message: value})
                    .then(response => {
                        const columns = [...this.columns];
                        const target = columns.find(item => item.key === key);
                        if (target) {
            ...
            fetchData() {
                ...
                // localStorage.setItem("newfiltersTableComments", JSON.stringify(this.filters));

                Vue.set(this.filters, 'typerewiews', this.typerewiews)

                axios.post(`/api/backend/comment/filter?page=${this.filters.pagination.current_page}`, this.filters)
                    .then(response => {
                        ...
                        if (this.typerewiews == 'event') {
                            ...
                            this.filters.pagination = response.data.comments
                            this.loading = false
            ...
            destroy(id) {
                swal({
                    ...
                    .then((willDelete) => {
                        if (willDelete) {
                            axios.delete(`/api/backend/comment/${id}`)
                                .then(response => {
                                    this.fetchData()
                                })
                                .catch(error => {
            ...
            moderate(id) {
                axios.get(`/api/backend/comment/${id}/moderate`)
                    .then(response => {
                        this.fetchData()
                        this.$message.success('Сохранено')
                    })
            ...
            togglePublished(id) {
                axios.post(`/api/backend/comment/${id}/toggle-published`)
                    .then(response => {
                        this.fetchData()
                        this.$message.success('Сохранено')
                    })
            ...
            toggleRejected(id) {
                axios.post(`/api/backend/comment/${id}/toggle-rejected`)
                    .then(response => {
                        this.fetchData()
                        this.$message.success('Сохранено')
                    })
