...
<script>
    ...
    export default {
        ...
        methods: {
            focusout: function() {
                console.log()
                let txt = this.tag.replace(/[^а-яa-z0-9\+\-\.\#/\s/g]/ig,''); // allowed characters
                if(txt) {
            ...
            fetch() {
                if (!this.loading) {
                    ...
                    axios.get(`/api/backend/category/${res[3]}`)
                        .then(response => {
            ...
            update() {
                if (!this.loading) {
                    ...
                    axios.put(`/api/backend/category/${res[3]}`, {category: this.category, seoEvent: this.seoEvent, seoCompany: this.seoCompany})
                        .then(response => {
                            this.errors = {}
                            this.category = response.data.category
            ...
            create() {
                if (!this.loading) {
                    this.loading = true
                    const message = this.$message.loading('Сохранение', 0)
                    axios.post(`/api/backend/category`, {category: this.category, seoEvent: this.seoEvent, seoCompany: this.seoCompany})
                        .then(response => {
                            this.errors = {}
                            this.category = response.data.category
                            this.$message.success('Успешно сохранено!')
                            location.href = `/admin/category/${response.data.category.slug}/edit`
                        })
                        .catch(error => {
                            this.$message.error('Ошибка')
                            this.errors = error.response.data.errors
            ...
            destroy() {
                if (!this.loading) {
                    ...
                    swal({
                        ...
                        .then((willDelete) => {
                            if (willDelete) {
                                axios.delete(`/api/backend/category/${this.category.slug}`)
                                    .then(response => {
