...
use App\Company;
use App\Event;
class commentPublishedForAuthor extends Notification
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        if ($this->comment->commented_type == Event::class) {
            $subject = "Ваш отзыв по акции успешно опубликован";
        } else if ($this->comment->commented_type == Company::class) {
            $subject = "Ваш отзыв по компании успешно опубликован";
        }
        ...
        return (new MailMessage)
            ...
            ->view('notifications.comment.commentPublishedForAuthorMail', ['data' => ['comment' => $this->comment, 'user' => $notifiable]]);
    }
}
